$(function () {
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				JY.Ajax.doRequest(null,uucpath +'/backstage/task/taskLog/delBatch',{chks:chks.toString()},function(data){
	...
	$('#exportXls').on('click', function(e) {
		...
		JY.Model.confirm("确认要导出数据吗?",function(){	
			window.location.href=uucpath+"/backstage/task/taskLog/exportXls";
		});	
	});
});
function search(){
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/task/taskLog/findByPage',null,function(data){
