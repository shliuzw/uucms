	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",uucpath +'/backstage/account/add',null,function(data){
				     that.dialog("close");
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){
				JY.Ajax.doRequest(null,uucpath +'/backstage/account/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});
		}
...
function loadRoleTree(){
	JY.Ajax.doRequest(null,uucpath +'/backstage/account/roleTree',null,function(data){
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/account/findByPage',null,function(data){
...
function check(accountId){
	...
	JY.Ajax.doRequest(null,uucpath +'/backstage/account/roleTree',null,function(data){
		$.fn.zTree.init($("#roleTree"),{view:{dblClickExpand:false,selectedMulti:false,nameIsHTML:true},data:{simpleData:{enable: true}},callback:{onClick:clickRole}},data.obj);
		JY.Ajax.doRequest(null,uucpath +'/backstage/account/find',{accountId:accountId},function(data){
		    setForm(data);
...
function del(accountId){
	JY.Model.confirm("确认删除吗？",function(){
		JY.Ajax.doRequest(null,uucpath +'/backstage/account/del',{accountId:accountId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
	...
	JY.Ajax.doRequest(null,uucpath +'/backstage/account/roleTree',null,function(data){
		...
		JY.Ajax.doRequest(null,uucpath +'/backstage/account/find',{accountId:accountId},function(data){
		    ...
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					JY.Ajax.doRequest("auForm",uucpath +'/backstage/account/update',null,function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});
					});
				}
		...
		buttons: [
		  {
			 ...
			 click: function() {
				 if(JY.Validate.form("resetPwdFrom")){
					 var that =$(this);
					 JY.Ajax.doRequest("resetPwdFrom",uucpath +'/backstage/account/resetPwd',null,function(data){
						 that.dialog("close");
			        	JY.Model.info(data.resMsg,function(){search();});		
					 });
				 }		
