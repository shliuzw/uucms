$(function () {
	...
	$('#syncFollowerBtn').on('click', function(e) {
		...
		JY.Model.confirm("确认要同步关注者数据吗?",function(){	
			JY.Model.loading();
			JY.Ajax.doRequest(null,uucpath +'/backstage/weixin/follower/syncFollower',"",function(data){
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/weixin/follower/findByPage',null,function(data){
...
function check(openid){
	cleanForm();
	JY.Ajax.doRequest(null,uucpath +'/backstage/weixin/follower/find',{openid:openid},function(data){
    		setForm(data);
