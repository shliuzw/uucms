...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/workflow/online/myTask/findTodoByPage',null,function(data){
...
function adjustTask(id,pId){
	cleanAdjustForm();
	JY.Ajax.doRequest(null,uucpath +'/backstage/workflow/online/myTask/findTask',{pId:pId},function(data){
		var obj=data.obj;
		setAdjustForm(obj);
		JY.Model.adjust("adjustDiv","调整申请"
			...
			if(JY.Object.notNull(description)){
				if(JY.Validate.form("adjustForm")){
					JY.Model.confirm("确认提交申请吗?",function(){	
						...
						ovar.pId=pId;
						JY.Ajax.doRequest("",uucpath +'/backstage/workflow/online/myTask/adjust/'+id,ovar,function(data){
			...
			JY.Model.confirm("确认放弃吗？",function(){	
				JY.Ajax.doRequest("",uucpath +'/backstage/workflow/online/myTask/complete/'+id,JY.Object.comVar(vars),function(data){
					JY.Model.info(data.resMsg,function(){search();that.dialog("close");});
				});
			});
		});
			...
			JY.Model.edit("rejectDiv","驳回",function(){	
				 ...
				 if(JY.Object.notNull(rejectReason)){
					 ...
					 JY.Model.confirm("确认驳回吗？",function(){			 
						 ...
						 ovar.pId=pId;
						 JY.Ajax.doRequest("",uucpath +'/backstage/workflow/online/myTask/reject/'+id,ovar,function(data){
							 JY.Model.info(data.resMsg,function(){search();that.dialog("close");});
						 });	
						});     
				 }else{
