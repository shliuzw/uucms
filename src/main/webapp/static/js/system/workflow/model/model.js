...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/workflow/model/findByPage',null,function(data){
...
function edit(modelId){
	JY.Model.confirm("确认要编辑模型吗?",function(){	
		window.open(uucpath+"/backstage/workflow/model/edit?modelId="+modelId); 
	});
}
function del(modelId){
	JY.Model.confirm("确认要删除模型吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/workflow/model/del',{modelId:modelId},function(data){
...
function exportFile(type){
	...
	window.open(uucpath+"/backstage/workflow/model/export/"+modelId+"/"+type);
}

function deployModel(modelId){
	JY.Model.confirm("确认要模型部署流程吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/workflow/model/deploy',{modelId:modelId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
