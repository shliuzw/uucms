		...
		JY.Model.edit("uploadDiv","上传流程模型",function(){
			...
			if(JY.Object.notNull(upVal)){
				...
				$("#uploadForm").ajaxSubmit({
                    type:'post',
                    url:uucpath +'/backstage/workflow/process/uploadModel',
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/workflow/process/findByPage',null,function(data){
...
function del(processDefinitionId){
	JY.Model.confirm("确认要删除流程定义吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/workflow/process/del',{processDefinitionId:processDefinitionId},function(data){
...
function convertToModel(processDefinitionId){
	JY.Model.confirm("确认要转换成模型吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/workflow/process/convertToModel',{processDefinitionId:processDefinitionId},function(data){
			 JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
...
function currentType(type,pdId){
	...
	layer.open({
	    ...
	    maxmin: true,
	    area: [width, height],
	    content: uucpath+"/backstage/workflow/process/resource/read?resourceType="+type+"&processDefinitionId="+pdId
	});
}
function cleanFile(){
