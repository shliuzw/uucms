$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增字典",function(){
			 if(JY.Validate.form("auForm")){
				 ...
				 $.ajax({type:'POST',url:uucpath+'/backstage/dataDict/add',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){
			        	 if(data.res==1){
			        		 that.dialog("close");      
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?",function(){	
			JY.Ajax.doRequest(null,uucpath +'/backstage/dataDict/delBatch',{chks:chks.toString()},function(data){
				JY.Model.info(data.resMsg,function(){search();});
			});
		});		
	});
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/dataDict/findByPage',null,function(data){
...
function del(id){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/dataDict/del',{id:id},function(data){
		 	 JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
function edit(id){
	cleanForm();
	JY.Ajax.doRequest(null,uucpath +'/backstage/dataDict/find',{id:id},function(data){
		setForm(data);
		JY.Model.edit("auDiv","修改字典",function(){
			if(JY.Validate.form("auForm")){
				...
				$.ajax({type:'POST',url:uucpath+'/backstage/dataDict/update',data:JSON.stringify(params),dataType:'json',contentType:"application/json",success:function(data,textStatus){
					   if(data.res==1){
					      that.dialog("close");
					      JY.Model.info(data.resMsg,function(){search();});
					   }else{
