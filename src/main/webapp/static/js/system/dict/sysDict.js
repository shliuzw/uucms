	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",uucpath +'/backstage/sysDict/add',null,function(data){
		        	that.dialog("close");      
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				JY.Ajax.doRequest(null,uucpath +'/backstage/sysDict/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});
		}	
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/sysDict/findByPage',null,function(data){
...
function check(id){
	cleanForm();
	JY.Ajax.doRequest(null,uucpath +'/backstage/sysDict/find',{id:id},function(data){
    	setForm(data);
...
function del(id){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/sysDict/del',{id:id},function(data){
	      	JY.Model.info(data.resMsg,function(){search();});
		});
	});	
}
function edit(id){
	...
	JY.Ajax.doRequest(null,uucpath +'/backstage/sysDict/find',{id:id},function(data){
    		...
    		JY.Model.edit("auDiv","修改",function(){
    			 if(JY.Validate.form("auForm")){
					 var that =$(this);
					 JY.Ajax.doRequest("auForm",uucpath +'/backstage/sysDict/update',null,function(data){
						 if(data.res==1){
			        		 that.dialog("close");
			        		 JY.Model.info(data.resMsg,function(){search();});
			        	 }else{
