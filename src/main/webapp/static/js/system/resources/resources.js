$(function () {
	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Ajax.doRequest("",uucpath +'/backstage/resources/listResources',{layer:layer},function(data){
			   //获取数据
			   var zNodes=data.obj;
			   //设置
			   ...
			   $.fn.zTree.init($("#preResourcesTree"), setting, zNodes);
				JY.Model.edit("auDiv","新增",function(){
					 ...
					 if(JY.Validate.form("auForm")){
						 var that =$(this);
						 JY.Ajax.doRequest("auForm",uucpath +'/backstage/resources/add',null,function(data){
				        	that.dialog("close");      
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				 JY.Ajax.doRequest(null,uucpath +'/backstage/resources/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				 });
			});	
		}	
...
function getbaseList(){
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/resources/listAllParentMenu',"",function(data){
...
function setNodes(id,parentid,nodes,html,level,permitBtn){
	if(nodes!=null&&nodes.length>0){
		...
		for(var i = 0;i<nodes.length;i++){
    		 ...
    		 if(i==nodes.length-1)
    			 html += "<img src='"+uucpath+"/static/css/system/system/images/joinbottom.gif' class='joinMiddle'/>";
    		 else
    			 html += "<img src='"+uucpath+"/static/css/system/system/images/join.gif' class='joinMiddle'/>";
...
function check(id){
	//清空表单
	cleanForm();
	JY.Ajax.doRequest(null,uucpath +'/backstage/resources/find',{id:id},function(data){
    		setForm(data);  
	...
	JY.Ajax.doRequest("",uucpath +'/backstage/resources/listResources',{layer:layer},function(data){
		   ...
		   JY.Ajax.doRequest(null,uucpath +'/backstage/resources/find',{id:id},function(data){
		    		...
		    		JY.Model.edit("auDiv","修改",function(){
						 ...
						 if(JY.Validate.form("auForm")){
							 var that =$(this);
							 JY.Ajax.doRequest("auForm",uucpath +'/backstage/resources/update',null,function(data){
					        		 that.dialog("close");
					        		 JY.Model.info(data.resMsg,function(){search();});		        		 	
							 });
						}
...
function del(id){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,uucpath +'/backstage/resources/del',{id:id},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});	
}
