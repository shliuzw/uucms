$(function () {
	...
	//加载菜单，从数据库获取，并封装成html
	getMenu();
	openMenu(1,"11","","系统首页","/backstage/adv");
//	TabControlAppend('shouye', '首页', 'false');
	//测试
	/*for(var i=1;i<18;i++){
...
function loadPro(){
	...
	if(typeof(settings) != "undefined"){
		...
		if(typeof(sjson.narWinMenu) != "undefined" &&sjson.narWinMenu=="1"){
			...
			$("#uucastle-settings-add-container").prop("checked",true);
		}else{
			$("#main-container").removeClass("container");	
			$("#uucastle-settings-add-container").prop("checked",false);
		}
		//缩放菜单
		if(typeof(sjson.sfMenu) != "undefined" &&sjson.sfMenu=="1"){
			$("#sidebar").addClass("menu-min");
			$("#uucastle-settings-sfMenu").prop("checked",true);
		}else{
			$("#sidebar").removeClass("menu-min");	
			$("#uucastle-settings-sfMenu").prop("checked",false);
		}
		//切换右菜单
		if(typeof(sjson.posMenu) != "undefined" &&sjson.posMenu=="1"){
			$("#indexBody").addClass("rtl");
			$("#uucastle-settings-rtl").prop("checked",true);
		}else{
			$("#indexBody").removeClass("rtl");	
			$("#uucastle-settings-rtl").prop("checked",false);
		}
	}else{
		var sjson={};
	...
	$("#perSetting").removeClass('hide').dialog({
		...
		buttons: [
		  {  
			...
			click: function() {	
				var that =$(this);
				 JY.Ajax.doRequest("perSettingFrom",uucpath +'/backstage/account/setSetting',null,function(data){
...
function perData(){
	//获取个人资料
	JY.Ajax.doRequest(null,uucpath +'/backstage/account/getPerData',null,function(data){
		if(data.res==1){
       		var o=data.obj;
       		//加载头像
       		$('#headpicRecommend li').removeClass("visiHot");
       		$("#headpicPreviewImg").attr('src',JY.Object.notNull(o.picUrl)?(o.picUrl):(uucpath+"/static/images/system/user/hpic0.jpg"));
       		//加载个人资料
       			  ...
       			  {  
       				...
       				click: function() {	
       					...
       					if($("#avatarSetting").hasClass("active")){
       						...
       						JY.Ajax.doRequest(null,uucpath +'/backstage/account/setHeadpic',{picUrl:picUrl},function(data){
	    				       JY.Model.info(data.resMsg); 
       						...
       						if(JY.Validate.form("perfileSettingFrom")){	
	       						JY.Ajax.doRequest("perfileSettingFrom",uucpath +'/backstage/account/setPerData',null,function(data){
		       				        JY.Model.info(data.resMsg); 
       						...
       						if(JY.Validate.form("pwSettingFrom")){
       							...
       							if(opwd!=npwd){
       								if(npwd==qpwd){	
       									JY.Ajax.doRequest(null,uucpath +'/backstage/account/preResetPWD',{opwd:opwd,npwd:npwd,qpwd:qpwd},function(data){
			       				        	JY.Model.info(data.resMsg); 
			       				        	that.dialog("close");	
       									});
           							}else{
...
// 打开菜单
// type:资源类型(1:menu；2:func；3:button)；
// id：资源id（数据格式：menu+id）；
// parentId：资源父id（数据格式：menu+id）；
// menuName：资源名称；resUrl：资源访问url，没有的话默认noset
function openMenu(type,id,parentId,menuName,resUrl){
	    if('1'!=type || "noset"==resUrl)
	    	return;
		else if('/'==resUrl.substring(0,1))
			TabControlAppend(id,menuName,uucpath+resUrl+"?menu="+id);
		else
			TabControlAppend(id,menuName,resUrl);
}
// 获取菜单
// 初始化index.jsp页面时，参数layer,ref都是空值。
// 首页中其他功能项中的菜单获取，参数layer,ref，在页面定义
function getMenu(layer,ref){
	$("#menu_li_id").empty();
	JY.Ajax.doRequest(null,uucpath +'/backstage/menu/getMenu',{layer:layer,ref:ref},function(data){
    	$("#menu_li_id").html(data.obj); 
	});
}
function logout(){
	JY.Model.confirm("确认要退出吗？",function(){window.location.href=uucpath+"/system_logout";});	
}
