$(function () {
	...
	$("#authorityDiv button").bind('click', function(){		
		...
		if(auth=='org'){
			...
			JY.Ajax.doRequest("",uucpath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:layer},function(data){
			...
			JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/listAuthorized',{id:roleId,layer:layer},function(data){
				$.fn.zTree.init($("#authorityTree"),{check: {enable: true,chkDisabledInherit: true,chkboxType:{"Y":"ps","N":"s"}},data:{simpleData:{enable: true}}},data.obj);	      
			});
		}else{
			...
			JY.Ajax.doRequest("",uucpath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:layer},function(data){
			         ...
			         JY.Model.edit("authorityDiv","机构组织授权",function(){
			        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
		 					...
		 					JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/saveOrgAuthorized',{id:orgId,aus:aus,layer:layer},function(data){
		 	 					 that.dialog("close");
	...
	$('#addOrgBtn').on('click', function(e) {
		...
		JY.Model.edit("auOrgDiv","新增机构组织",function(){
			 if(JY.Validate.form("auOrgForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auOrgForm",uucpath +'/backstage/org/role/addOrg',null,function(data){
						 that.dialog("close"); 
	...
	$('#editOrgBtn').on('click', function(e) {
		...
		//loadPreOrgTree();
		var orgId=$("#baseForm input[name$='orgId']").val();
		JY.Ajax.doRequest(null,uucpath+'/backstage/org/role/findOrg',{id:orgId},function(data){
			if(data.res==1){
				// 页面属性赋值数据
				setOrgForm(data);    
				JY.Model.edit("auOrgDiv","修改组织",function(){
					 if(JY.Validate.form("auOrgForm")){
						 var that =$(this);
						 JY.Ajax.doRequest("auOrgForm",uucpath +'/backstage/org/role/updateOrg',null,function(data){
								 that.dialog("close"); 
				        		 JY.Model.info(data.resMsg,function(){refreshOrgTree();});     
						 }); 
					 }	
	...
	$('#delOrgBtn').on('click', function(e) {
		...
		JY.Model.confirm("确认要删除【"+ptreeNode.name+"】吗？",function(){		
			JY.Ajax.doRequest(null,uucpath+'/backstage/org/role/delOrg',{id:orgId},function(data){
		     ...
		     var ptreeNode=treeObj.getNodeByParam("id",orgId,null);*/
				JY.Model.edit("auDiv","新增角色",function(){
				 if(JY.Validate.form("auForm")){
					 var that =$(this);			
					 JY.Ajax.doRequest("auForm",uucpath +'/backstage/org/role/add',{orgId:orgId},function(data){
			        		 that.dialog("close");      
			        		 JY.Model.info(data.resMsg,function(){search();});
					 }); 
				 }	
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的角色吗?",function(){
				JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});	
		}	
...
function loadOrgTree(){
	JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/getOrgTree',null,function(data){
		//设置
		$.fn.zTree.init(
			$("#orgTree"),
...
function loadPreOrgTree(){
	JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/getPreOrgTree',null,function(data){
		//设置
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",uucpath +'/backstage/org/role/findByPage',null,function(data){
...
function authorized(id){
	...
	var layer=$("#authorityDiv input[name$='layer']").val();
	JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/listAuthorized',{id:id,layer:layer},function(data){
	         //获取数据
	    	 var zNodes=data.obj;
	         //设置
	         ...
	         JY.Model.edit("authorityDiv","角色授权",function(){
	        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
 					...
 					var that =$(this);
 					layer=$("#authorityDiv input[name$='layer']").val();
 					JY.Ajax.doRequest(null,uucpath +'/backstage/org/role/saveAuthorized',{id:id,aus:aus,layer:layer},function(data){
 	 					  that.dialog("close");
 	 					  JY.Model.info(data.resMsg,function(){search();});			        		 	
 					});
	         });
...
function check(id){
	//清空表单
	cleanForm();
	JY.Ajax.doRequest(null,uucpath+'/backstage/org/role/find',{id:id},function(data){
		setForm(data);    
...
function edit(id){
	...
	JY.Ajax.doRequest(null,uucpath+'/backstage/org/role/find',{id:id},function(data){
    		...
    		JY.Model.edit("auDiv","修改角色",function(){
    			 if(JY.Validate.form("auForm")){
					 var that =$(this);
					 JY.Ajax.doRequest("auForm",uucpath+'/backstage/org/role/update',null,function(data){
			        		 that.dialog("close");
			        		 JY.Model.info(data.resMsg,function(){search();});      				        		 	
					 });
				}
...
function del(id){
	JY.Model.confirm("确认删除角色吗？",function(){	
		JY.Ajax.doRequest(null,uucpath+'/backstage/org/role/del',{id:id},function(data){
			 JY.Model.info(data.resMsg,function(){search();});
		});
	});	
}
