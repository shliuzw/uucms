	...
	$('#addPosBtn').on('click', function(e) {
		...
		JY.Model.edit("auPosDiv","新增职务",function(){
			 ...
			 if(JY.Validate.form("auPosForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auPosForm",uucpath +'/backstage/org/position/addPos',null,function(data){	
					that.dialog("close"); 
		...
		JY.Model.edit("arrangeAccDiv","安排人员",function(){
			...
			if(chks.length==0) {
				...
				JY.Model.confirm("确认选中的人员吗?",function(){	
					JY.Ajax.doRequest("",uucpath +'/backstage/org/position/arrangeAcc',{posId:posId,chks:chks.toString()},function(data){
						that.dialog("close"); 
...
function refreshOrgTree(){
	 JY.Model.loading();
	 loadPreOrgTree();
	 JY.Ajax.doRequest(null,uucpath +'/backstage/org/position/getOrgAndPositionTree',null,function(data){
			//设置
...
function loadPreOrgTree(){
	JY.Ajax.doRequest(null,uucpath +'/backstage/org/position/getPreOrgTree',null,function(data){
		//设置
...
function getOrgList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseOrgForm",uucpath +'/backstage/org/position/findOrgByPage',null,function(data){
		 $("#baseOrgTable tbody").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
...
function checkPos(id){
	cleanPosForm();
	JY.Ajax.doRequest(null,uucpath+'/backstage/org/position/findPos',{id:id},function(data){
    	setPosForm(data);    
	...
	JY.Ajax.doRequest(null,uucpath+'/backstage/org/position/findPos',{id:id},function(data){
    		...
    		JY.Model.edit("auPosDiv","修改职务",function(){
    			 ...
    			 if(JY.Validate.form("auForm")){
					 var that =$(this);
					 JY.Ajax.doRequest("auPosForm",uucpath+'/backstage/org/position/updatePos',null,function(data){
			        	that.dialog("close");
...
function delPos(id){
	JY.Model.confirm("确认删除职务吗？",function(){	
		JY.Ajax.doRequest(null,uucpath+'/backstage/org/position/delPos',{id:id},function(data){
			JY.Model.info(data.resMsg,function(){refreshOrgTree();getOrgList();});
		});
	});	
}
...
function loadArrangeAcc(init){
	...
	JY.Model.loading();
	var posId=$("#basePosForm input[name$='posId']").val();
	JY.Ajax.doRequest("arrangeAccForm",uucpath +'/backstage/org/position/findArrangeAccByPage',{id:posId},function(data){
...
function getPosList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("basePosForm",uucpath +'/backstage/org/position/findPosByPage',null,function(data){
		 $("#basePosTable tbody").empty();
    		 var obj=data.obj;
        	 var list=obj.list;
        	 var results=list.results;
...
function delAccPos(id){
	JY.Model.confirm("确认移除人员职务吗？",function(){	
		JY.Ajax.doRequest(null,uucpath+'/backstage/org/position/delAccPos',{accId:id},function(data){
			JY.Model.info(data.resMsg,function(){getPosList(1);});
		});
	});	
}
